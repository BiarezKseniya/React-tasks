...
import styles from '@/components/stats-range/StatsRange.module.css';
...
const StatsRange = ({ skillName, skillValue }: StatsRangeProps) => {
  ...
  return (
    <div className={styles['stats-range']}>
      <label
        className={styles['stats-range__label']}
      >{`${skillName}: ${skillValue}`}</label>
      <input
        className={styles['stats-range__range']}
        ref={rangeRef}
        type="range"
        value={skillValue}
        min={0}
